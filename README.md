...
Soundpad Quick Cut 解决了 Soundpad 用户的一个高频痛点：从视频里截音频片段太麻烦。传统的做法是用 Premiere 或剪映打开视频、切片段、导出音频、再拖进 Soundpad，流程重且慢。

这个工具把上述流程压缩到**一个窗口里完成**：拖入视频 → 波形上选段 → 一键导出。适合经常从影视素材、直播录屏里快速截取音效的用户。


## 功能

- 拖拽 / 按钮导入视频与音频（MP4、MKV、MOV、AVI、WebM、MP3、WAV、FLAC、M4A 等），支持多文件批量导入至素材库
- 视频预览 + 音频波形可视化，原生滚动条平移，`Ctrl+滚轮` 缩放时间轴
- `I` / `O` 键打入出点，或鼠标拖拽波形直接选区，两端手柄微调
- 多片段工作流：一个素材可攒下多个片段（`A` 添加），素材库展开管理，一键全部导出到 Soundpad
- 撤销 / 重做（`Ctrl+Z` / `Ctrl+Shift+Z`），`[` `]` 微调入出点，`Home` / `End` 跳头尾
- 放大时间轴时自动提取高分辨率波形；波形悬停显示时间，选区上标注时长
- 选区播放（自动跳入点 → 播到出点停止 → 再播重新从入点开始）
- 循环开关 + 音量滑杆，反复打磨片段更方便
- 逐帧 / 逐秒步进（`←` `→` / `Shift+←` `Shift+→`），支持长按连击
- 导出 WAV / MP3，格式自动记忆，导出后自动打开所在文件夹；导出过程可取消
- 淡入淡出 / 增益调节（导出时应用），参数自动记忆；导出后可先在应用内试听
- 片段命名：默认取视频名前 5 字，同一素材多次导出自动追加 `_01`、`_02`；"导出到 Soundpad" 的文件直接写入 Soundpad 声音目录，不会因临时文件清理而失效
- Soundpad 集成：实时读取 Soundpad 分类与连接状态，分类不存在时自动创建；中文文件名以系统编码正确推送
- 素材库：左侧面板管理多段素材，各自独立保留入出点
- 素材库自动保存（重启后恢复素材与选区，波形按需重新分析）
- 首次启动使用引导，顶栏问号按钮可随时重新打开
- 菜单栏隐藏，界面紧凑专注编辑区域
